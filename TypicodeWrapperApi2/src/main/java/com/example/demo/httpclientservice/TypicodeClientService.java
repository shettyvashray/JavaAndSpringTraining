...
//	private final String API_URL = "https://jsonplaceholder.typicode.com/";
	...
	private List<Comments> getCommentsForPost(int postId) throws IOException {
        HttpGet request = new HttpGet(API_URL + "/comments?postId=" + postId);
        HttpResponse response = httpClient.execute(request);
        Comments[] comments = objectMapper.readValue(response.getEntity().getContent(), Comments[].class);
	...
	public List<Posts> getAllPostsWithComments() throws IOException {
		HttpGet request = new HttpGet(API_URL + "/posts");
		HttpResponse response = httpClient.execute(request);
		Posts[] posts = objectMapper.readValue(response.getEntity().getContent(), Posts[].class);
	...
	private List<Photos> getPhotosForAlbum(int albumId) throws IOException {
        HttpGet request = new HttpGet(API_URL + "/photos?albumId=" + albumId);
        HttpResponse response = httpClient.execute(request);
        Photos[] photos = objectMapper.readValue(response.getEntity().getContent(), Photos[].class);
	...
	public List<Albums> getAllAlbumsWithPhotos() throws IOException {
		HttpGet request = new HttpGet(API_URL + "/albums");
		HttpResponse response = httpClient.execute(request);
		Albums[] albums = objectMapper.readValue(response.getEntity().getContent(), Albums[].class);
		for (Albums album : albums) {
			album.setPhotos(getPhotosForAlbum(album.getId()));
		}
